...
use crate::utils::{config::Config,
                   error::MyError,
                   chain::mint_havenmarket,};
use crate::utils::log_status::log_status;


pub async fn havenmarket (
    session: &HttpClient,
    web3_client: &Client,
    ...
    if let Some((dummy_id_str, signature)) = mint_values {
        ...
        // Mint NFT HavenMarket
    if config.settings.mint_havenmarket {
        match mint_havenmarket(&key_without_prefix, &address_without_prefix, web3_client, dummy_id, signature.as_str(), &config, index).await {
...
async fn check (
    ...
    // -------------
    let url0 = format!("https://havenmarket.xyz/api/user?address={}&chain_id=204", address);

    let request0 = Request::builder()
        .method("GET")
        .uri(&url0)
        .header("authority", "havenmarket.xyz")
        .header("accept", "application/json, text/plain, */*")
        .header("content-type", "application/json")
        .header("origin", "hhttps://tests.havenmarket.xyz")
        .header("referer", "https://tests.havenmarket.xyz/")
        .header("sec-ch-ua", " ")
...
async fn connect_tg (
    ...
    // -------------
    let url0 = "https://havenmarket.xyz/api/telegram/join".to_string();
...
async fn check_mint (
    ...
    // -------------
    let url0 = "https://havenmarket.xyz/api/mint/sign0".to_string();
    let mut data: HashMap<String, serde_json::Value> = HashMap::new();
    data.insert("nft_contract".to_string(), json!(NFT_CONTRACT_HV));
    data.insert("mint_contract".to_string(), json!(MINT_CONTRACT_HV));
...
async fn connect_ds (
    ...
    // --------------
    let url_ds = format!("https://havenmarket.xyz/api/discord/verify?provider=discord&address={}&chain_id=204", address);
    ...
    if let Some(location) = response_ds.headers().get(http::header::LOCATION) {
        ...
        if result?.contains("Error") {
            log_status(index, &address, "havenmarket_ds", "error").await;
        } else {
            log_status(index, &address, "havenmarket_ds", "ok").await;}
...
async fn connect_tw (
    ...
    let url_ds = format!("https://havenmarket.xyz/api/twitter/verify?address={}&chain_id=204", address);
    ...
    if let Some(location) = response_tw.headers().get(http::header::LOCATION) {
        ...
        if result.clone()?.contains("Error") {
            log_status(index, &address, "havenmarket_tw", "error").await;
        } else {
            log_status(index, &address, "havenmarket_tw", "ok").await;}
