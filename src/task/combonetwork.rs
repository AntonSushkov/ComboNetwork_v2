...
use crate::utils::{config::Config,
                   error::MyError,
                   chain::mint_combonetwork,
                   log_status::log_status};



pub async fn combonetwork (
    session: &HttpClient,
    web3_client: &Client,
    ...
    // let logger = Loggers::new("Logs/output.csv");
    ...
    if let Some((dummy_id_str, signature)) = mint_values {
        ...
        // Mint ComboNetwork NFT
        if config.settings.mint_combonetwork {
            match mint_combonetwork(&key_without_prefix, &address_without_prefix, &web3_client, dummy_id, signature.as_str(), &config, index).await {
...
async fn check_mint (
    ...
    let mut data: HashMap<String, serde_json::Value> = HashMap::new();
    data.insert("nft_contract".to_string(), json!(NFT_CONTRACT_CMB));
    data.insert("mint_contract".to_string(), json!(MINT_CONTRACT_CMB));
...
async fn connect_ds (
    ...
    if let Some(location) = response_ds.headers().get(http::header::LOCATION) {
        ...
        if result?.contains("Error") {
            log_status(index, &address, "combonetwork_ds", "error").await;
        } else {
            log_status(index, &address, "combonetwork_ds", "ok").await;}
...
async fn connect_tw (
    ...
    if let Some(location) = response_tw.headers().get(http::header::LOCATION) {
        ...
        if result.clone()?.contains("Error") {
            log_status(index, &address, "combonetwork_tw", "error").await;
        } else {
        log_status(index, &address, "combonetwork_tw", "ok").await;}
